...
class BuildCursorRulesTests(unittest.TestCase):
    def run_builder(self, skills_dir: Path, out_dir: Path) -> subprocess.CompletedProcess[str]:
        return subprocess.run(
            [sys.executable, str(BUILDER), "--skills-dir", str(skills_dir), "--out-dir", str(out_dir)],
            check=False,
            capture_output=True,
            text=True,
        )
